...
public class MutationCorrelated implements IMutation {
	...
	public double calculate_c_covarianceValue(int i_index, int j_index) {
		...
		if (i_index == j_index) {
			...
			return 0.5
					* (Math.pow(individual.getSigma_mutation_step_sizes()[i_index], 2) - Math.pow(
							individual.getSigma_mutation_step_sizes()[j_index], 2))
	...
	public double calculateSigmaMutationStepSize(double previous_sigma_mutation_step_size) {
		// see ch04.pdf, p.22
		return previous_sigma_mutation_step_size
				* Math.exp(calculateTauApostropheOverallLearningRate() * player11.getRnd().nextGaussian()
						+ calculateTauCoordinateWiseLearningRate() * player11.getRnd().nextGaussian());
	}
