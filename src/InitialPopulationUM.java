 ...
 *
 */
public class InitialPopulationUM implements IInitialPopulation {

	private static final double INITIAL_MUTATION_VALUE = 0.8;
	private int populationSize;

	public InitialPopulationUM(int populationSize) {
	...
	public Population createInitialPopulation() {
		...
		while (population.getPopulationSize() < populationSize) {
			...
			for (int i = 0; i < randomDna.length; i++) {
				// bounds are [-5.0,5.0[
				randomDna[i] = (AlgorithmUM.ran.nextDouble() * player11.F_DIMENSION) - player11.MAX_SEARCH_VALUE;
			}
