    ...
    public static double[][] tuneESParameters(TuningEvaluation eval, int NUMBER_OF_RUNS){
        ...
        for(int MU = 2; MU<10; MU++){
            for(int LAMBDA = 10; LAMBDA<50; LAMBDA+=2){
                AbstractEA algo = new AlgorithmES(MU, LAMBDA, new SelectionAbsoluteFitness(MU));
        ...
        for(double ALPHA = 0.1; ALPHA<2; ALPHA+=0.1){
            AbstractEA algo = new AlgorithmES((int)best_result_mu_lambda[1],(int)best_result_mu_lambda[2], new SelectionAbsoluteFitness((int)best_result_mu_lambda[1]), ALPHA);
            double average_result = runEvaluation(eval, algo, NUMBER_OF_RUNS);
        ...
        for(double EPSILON = 0.000001; EPSILON<=0.1; EPSILON*=10){
            AbstractEA algo = new AlgorithmES((int)best_result_mu_lambda[1],(int)best_result_mu_lambda[2],new SelectionAbsoluteFitness((int)best_result_mu_lambda[1]),best_result_alpha[1],EPSILON);
            double average_result = runEvaluation(eval, algo, NUMBER_OF_RUNS);
