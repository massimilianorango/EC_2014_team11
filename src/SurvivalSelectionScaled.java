...
public class SurvivalSelectionScaled implements ISurvivalSelection {
	...
	public static final double S_INDIVIDUAL_ADVANTAGE = 2;
	private final int MAX_POPULATION_SIZE;

	public SurvivalSelectionScaled(int maxPopulationSize) {
	...
	public void selectSurvivals(Population population, ArrayList<Individual> children) {
		...
		for (int i = 0; i < children.size(); i++) {
			int rank = i;
			probabilitySum += (2 - S_INDIVIDUAL_ADVANTAGE) / mu_count + 2.0 * rank * (S_INDIVIDUAL_ADVANTAGE - 1)
					/ (mu_count * (mu_count - 1));
